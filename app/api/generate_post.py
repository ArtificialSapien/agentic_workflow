...
def generate_post(
    ...
    generated_text, image_url, video_url, meme_url = run_agent(agent = agent, initial_input = initial_input)
...
lim = AzureDallE3ImageGenerator(api_key=os.getenv('AZURE_OPENAI_API_KEY'), url=os.getenv('AZURE_OPENAI_DALLE3_ENDPOINT'))
...
def image_generator(state: AgentState):
    ...
    if state["generate_image"] == True:
        prompt_for_image_generation: str = llm.invoke(prompt_for_instructing_image_generation).content
...
def run_agent(agent,initial_input):
    ...
    return final_state['generated_text'], final_state['generated_image_url'], 'whatever2', 'whatever3'
